...
扫描会递归列出支持的文档，不再按目录深度和文档数量截断；以 `.` 开头的文档和普通子目录也会列出。软链接以及 `.git`、`node_modules`、`target`、`dist` 等目录仍会跳过。PDF 支持载入 250 MB 以内的文件，其他文档限制为 50 MB；超限文件仍显示在列表中并标明原因。无法读取的子目录会有提示，批量导入失败会显示具体文件名和原因。返回文件夹视图、执行全部导入时会重新扫描，避免遗漏磁盘上新增的文档。
...
导入的 PDF 支持 Ctrl+F 实时查找、逐项计数和 Enter / Shift+Enter 循环跳转。匹配文字显示黄色高亮，当前结果显示橙色高亮；中文跨文字片段、换行、缩放和旋转均保持定位。文字层提取失败不会覆盖已渲染的页面，搜索会提示无法搜索的页数；纯图片 PDF 需要先经过 OCR 才能查找文字。中文字体映射、标准字体和图片解码资源随应用离线打包。
...
这是界面与文档引擎接入阶段，尚未完成需求中的全部 19 个阶段。导入内容保存在此设备的工作区（文本草稿使用 localStorage，PDF / DOCX 原文件副本使用 IndexedDB），当前保存与重命名、移动、移除操作针对工作区记录；Rust 文件系统写回尚未接入。“打开文件夹”只读取文档，不持续监听目录变化；返回文件夹视图和批量导入前会重新扫描，也可以点击“重新扫描”刷新。

Markdown 可导出 `.md`；PDF 和未编辑 DOCX 可导出原文件；编辑后的 Word 暂导出 HTML，尚未实现 DOCX 编辑稿回写和旧 `.doc` 格式。Markdown 的 Mermaid / 数学公式仍待接入。AI 在 Windows 桌面端通过用户配置的模型服务调用；浏览器仅预览设置界面。AI 聊天记录按文档加密保存在应用数据目录，可跨重启恢复；直接把编辑建议写回文档尚未实现。AI 附件限单条消息 4 个文件、单个 64 KB 文本、合计 128 KB。工作区单文件导入限制为 PDF 250 MB，Markdown / DOCX 50 MB。

  ...
  overlays.menu(event, [
    {
      ...
      icon: 'lucide:x',
      shortcut: 'Ctrl W',
      action: () => workspace.close(pane.id, tab.id),
    },
    {
      label: '关闭其他标签页',
      action: () => {
        for (const item of [...pane.tabs])
          if (item.id !== tab.id && !item.pinned)
            workspace.close(pane.id, item.id)
      },
    },
    {
      label: '关闭右侧标签页',
      action: () => {
        for (const item of pane.tabs.slice(
          pane.tabs.findIndex(item => item.id === tab.id) + 1
        ))
          if (!item.pinned) workspace.close(pane.id, item.id)
      },
    },
    {
      label: '关闭全部标签页',
    ...
    },
    {
      ...
      icon: 'lucide:pin',
      divider: true,
      action: () => workspace.togglePin(pane.id, tab.id),
    },
    {
      label: '在右侧分屏',
    ...
    },
    {
      ...
      shortcut: 'Ctrl ⇧ T',
      divider: true,
      disabled: !workspace.closedTabs.length,
  ...
  >
    ...
    >
      ...
      >
        <AppIcon
          :name="
            documentTypes[documents.get(tab.documentId)?.kind ?? 'markdown']
              .icon
          "
          :size="14"
        />
        <span class="tab-name truncate">{{
          documents.get(tab.documentId)?.name
        }}</span>
        <AppIcon
          v-if="tab.pinned"
          name="lucide:pin"
        ...
        <button
          v-else
          class="tab-close hover:bg-hover [&.dirty]:before:bg-muted grid size-[18px] shrink-0 place-items-center rounded opacity-0 group-focus-within:opacity-100 group-hover:opacity-100 group-[.active]:opacity-100 [&.dirty]:before:size-[5px] [&.dirty]:before:rounded-full [&.dirty:hover]:before:hidden [&.dirty:hover>svg]:block [&.dirty>svg]:hidden"
          :class="{ dirty: documents.get(tab.documentId)?.dirty }"
          :aria-label="`关闭 ${documents.get(tab.documentId)?.name}`"
          @click.stop="workspace.close(pane.id, tab.id)"
        >
          <AppIcon
            name="lucide:x"
            :size="12"

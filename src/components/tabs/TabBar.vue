...
function endDrag() { workspace.drag = null; dropIndex.value = null }
function createDocument() { workspace.activate(props.pane.id); actions.create() }
...
<template>
  <div class="tab-bar flex h-9 shrink-0 select-none items-center gap-[3px] border-b border-line bg-canvas px-2" @dragover.prevent @drop="drop" @dragleave="dropIndex = null">
    ...
    <IconButton icon="lucide:plus" label="新建 Markdown" @click="createDocument" />

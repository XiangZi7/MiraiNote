  ...
  <Teleport to="body"
    ...
    >
      <div
        ...
        role="menu"
        aria-label="操作菜单"
        class="border-line bg-elevated shadow-floating fixed max-h-[calc(100vh-16px)] min-w-[220px] overflow-auto rounded-[10px] border p-1.5"
      ...
      >
        <template
          ...
          ><div
            v-if="item.divider && index"
            class="bg-line mx-1 my-1 h-px"
            role="separator"
          />
          <button
            role="menuitem"
            :disabled="item.disabled"
            class="enabled:hover:bg-hover focus-visible:bg-hover flex h-8 w-full items-center gap-2.5 rounded-[5px] px-2.5 text-left text-xs outline-none disabled:opacity-40"
            :class="item.danger ? 'text-danger' : 'text-secondary'"
            @click="execute(item)"
          >
            <AppIcon
              v-if="item.icon"

...
<template>
  <section class="flex h-full flex-col overflow-hidden bg-surface" :aria-label="title">
    ...
    <div class="mt-8 flex items-center gap-2 text-[11px] text-muted"><AppIcon name="lucide:folder-open" :size="14" /><span>示例工作区</span><span class="mx-1 text-faint">·</span><button class="hover:text-accent" @click="actions.openFiles">导入本地文档</button></div></div></div>
  </section>
</template>

...
export function applyMarkdownAction(
  ...
  if (wrap) {
    ...
    if (prefix) {
      ...
      const insert = action.startsWith('h')
        ? prefix + content.replace(/^#{1,6}\s/, '')
        : content
            .split('\n')
...
export function createEditor(
  ...
  onPosition: (cursor: number, scroll: number) => void
): EditorView {
    ...
    state: EditorState.create({
      ...
      extensions: [
        ...
        EditorView.domEventHandlers({
          scroll: (_, view) => {
            onPosition(view.state.selection.main.head, view.scrollDOM.scrollTop)
          },
        }),

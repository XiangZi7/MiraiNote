...
const pages = computed(() =>
  Array.from({ length: pdf.value?.numPages ?? 0 }, (_, index) => index + 1)
)
...
async function fit(whole = false) {
  ...
  const size = page.getViewport({
    scale: 1,
    rotation: props.tab.position.rotation,
  })
  zoom(
    Math.min(
      (width.value - 64) / size.width,
      whole ? (height.value - 52) / size.height : 2.5
    ) * 100
  )
}
async function find() {
...
async function search() {
  ...
  for (let offset = 1; offset <= pdf.value.numPages && !disposed; offset++) {
    const number =
      ((props.tab.position.page - 1 + offset) % pdf.value.numPages) + 1
...
<template>
  <div class="bg-canvas flex h-full flex-col">
    ...
    >
      <aside
        v-if="thumbnails"
        class="border-line bg-inspector w-[146px] shrink-0 overflow-auto border-r px-5 py-5"
        aria-label="PDF 缩略图"
      >
        <button
          v-for="number in pages"
          :key="number"
          class="mb-5 block border-2 p-0.5"
          :class="
            number === tab.position.page
              ? 'border-accent'
              : 'border-transparent'
          "
          :aria-label="`跳转第 ${number} 页`"
          @click="go(number)"
        >
          <PdfCanvas
            :pdf="pdf"
      ...
      <div
        ref="viewport"
        class="bg-sidebar min-w-0 flex-1 overflow-auto px-8 py-[26px]"
      >
        <div class="mx-auto w-max shadow-md">
          <PdfCanvas
            :pdf="pdf"

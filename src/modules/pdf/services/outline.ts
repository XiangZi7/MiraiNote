...
export async function readPdfOutline(pdf: PDFDocumentProxy): Promise<PdfOutlineEntry[]> {
  ...
  async function walk(nodes: Nodes, level: number) {
    for (const node of nodes) {
      ...
      } catch { /* A broken bookmark must not prevent reading the rest of the PDF. */ }
      entries.push({ id: String(entries.length), title: node.title || '未命名书签', level, page, disabled: !page })

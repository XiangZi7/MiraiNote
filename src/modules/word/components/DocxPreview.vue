...
onMounted(async () => {
  try {
    ...
    if (!disposed)
      error.value =
        reason instanceof Error ? reason.message : 'Word 文档解析失败'
  }
})
onBeforeUnmount(() => {
  disposed = true
...
<template>
  <div
    ref="container"
    class="mx-auto w-max [&>section]:mb-6 [&>section]:bg-white [&>section]:shadow-md"
    aria-label="Word 原始排版预览"
  >
    <p
      v-if="error"
      role="alert"
      class="text-danger p-10"
    >
      {{ error }}
    </p>
  </div>

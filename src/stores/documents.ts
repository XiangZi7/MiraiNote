  ...
  return (
    ...
    value.every(
      item =>
        ...
        ['createdAt', 'modifiedAt', 'openedAt'].every(
          key =>
...
export const useDocumentsStore = defineStore('documents', () => {
  const documents = ref(
    loadJson('documents', exampleDocuments(), validDocuments)
  )
  const byId = computed(
    () => new Map(documents.value.map(item => [item.id, item]))
  )
  ...
  function persist() {
    persistJson('documents', documents.value)
  }
  ...
  function create() {
    ...
    const index =
      documents.value.filter(doc => doc.name.startsWith('未命名')).length + 1

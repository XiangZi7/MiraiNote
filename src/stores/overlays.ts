...
export const useOverlaysStore = defineStore('overlays', () => {
  const state = reactive({
    ...
    prompt: null as { title: string; label: string; value: string; confirm: string; danger: boolean; action: (value: string) => void | Promise<void> } | null,
  })
  function toast(message: string, error = false) {
    const id = crypto.randomUUID()
    state.toasts.push({ id, message, error })
    window.setTimeout(() => { state.toasts = state.toasts.filter(item => item.id !== id) }, error ? 7000 : 3500)
  }
  function menu(event: MouseEvent, items: MenuItem[]) {
    state.menu = { x: event.clientX, y: event.clientY, items }
  }
  function close() { state.palette = null; state.settings = false; state.help = false; state.menu = null; state.prompt = null }
